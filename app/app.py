...
@st.cache_data
def load_data():
    df_ml = pd.read_csv(os.path.join(BASE_DIR, 'data', 'cleaned_dataset_for_ml.csv'))
    df_res = pd.read_csv(os.path.join(BASE_DIR, 'models', 'model_results.csv'))
    df_insights = pd.read_csv(os.path.join(BASE_DIR, 'data', 'data_insights.csv'))
...
if page == "🏠 Home":
    ...
    **What you can do here:**
    ...
    """)
    img_path = os.path.join(BASE_DIR, 'assets', 'crop_yield.jpg')
...
# --- PAGE 4: Yield Prediction ---
    ...
    st.title("🔮 Cereal Crop Yield Prediction")
    ...
    try:
        ...
        selector = joblib.load(os.path.join(BASE_DIR, "models", "selector.pkl"))
        high_vif_cols = joblib.load(os.path.join(BASE_DIR, "models", "high_vif_cols.pkl"))
        min_year = joblib.load(os.path.join(BASE_DIR, "models", "min_year.pkl"))
    ...
    with st.form("prediction_form"):
        ...
        with col3:
            year = st.slider("Year (B.S.)", min_value=2067, max_value=2085, value=2081)
            st.caption("Forecasts reliable up to 5 years beyond historical data (2079).")
...
# --- PAGE 5: Model Explainability ---
    ...
    with col1:
        ...
        try:
            shap_imp = pd.read_csv(os.path.join(BASE_DIR, "plots", "shap_importance.csv"))
    ...
    * **X-axis** shows whether that value pushed the predicted yield higher or lower.
    """)
    try:
        st.image(os.path.join(BASE_DIR, "plots", "shap_summary.png"), use_container_width=True)
    except:
